...
inputNumberTwo: .asciiz "Please input second number Ex: 0234 \n"
...
handleInInterruptRowOne:
  ...
  case_interrupt_0:
    ...
    addi $a3,$a2,0    # left=right
    addi $a2,$zero,0  # left = 0
    mul $s1,$s1,10
...
handleInInterruptRowThree:
  ...
  sw $ra,0($sp) 
  li $t1,IN_ADDRESS_HEXA_KEYBOARD
  li $t3,0x84     # Kich hoat interrupt, cho phep bam phim o hang 2
  sb $t3,0($t1)
  li $t1,OUT_ADDRESS_HEXA_KEYBOARD
  lb $t3,0($t1)   # Nhan byte the hien vi tri cua phim duoc bam trong hang 2
  case_interrupt_8:
...
handleInInterruptRowFour:
  ...
  sw $ra,0($sp) 
  li $t1,IN_ADDRESS_HEXA_KEYBOARD
  li $t3,0x88     # Kich hoat interrupt, cho phep bam phim o hang 2
  sb $t3,0($t1)
  li $t1,OUT_ADDRESS_HEXA_KEYBOARD
  lb $t3,0($t1)   # Nhan byte the hien vi tri cua phim duoc bam trong hang 2
  case_interrupt_c:
